...
var Cdn = module.exports = function Cdn(options, callback) {
  ...
  function middleware(req, res, next) {
    res.cdn = that.cdn(req.url);
    next();
  };
  middleware.cdn = this.cdn.bind(this);
...
Cdn.prototype.cdn = function(relativeUrl) {
  var that = this,
      relativePath = relativeUrl.replace(/^https?:\/\/[^\/]+\/|^\//, '')
                                .replace(/\/[^\/]+$/, '/');

  return function(filename, immediate) {
    filename = filename.toString();
    var absolute = /^\//.test(filename),
        _filename = absolute ?
                    filename.replace(/^\//, '')
                    :
                    filenarelativePath +
    ...
    that.store(_filename, immediate);

    return that.hash[_filename] ||
           filename;
  };
};
