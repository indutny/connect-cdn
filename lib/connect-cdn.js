...
Cdn.prototype.init = function() {
  ...
  this.service.getContainer(this.containerName, function(err, container) {
    if (err || !container) {
      var c = new cloudfiles.Container(that.service, {
        name: that.containerName,
        cdnEnabled: true
      });

      return that.service.createContainer(c, function(err, container) {
        if (err) throw err;
        end(container);
      });
    }
...
Cdn.prototype.store = function(filename) {
  ...
  var srcname = this.root + filename.replace(/^\/+/, '');
